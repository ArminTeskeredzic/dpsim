...
void scenario_B_step_A(String simName, Real timeStep, Real finalTime, Bool startFaultEvent, Bool endFaultEvent, Real startTimeFault, Real endTimeFault, Bool useVarResSwitch, Real cmdInertia_G1, Real cmdDamping_G1) {
	...
	dummy_load_bus_psha_PF->setParameters(1e-15, 1e-15, 220e3);
	...
	line_a_load_PF->setParameters(1e-12, 1e-7, 1e-12, 1e-12);
	...
	line_b_load_PF->setParameters(1e-12, 1e-7, 1e-12, 1e-12);
	...
	*/
	...
	//shunt_SR_acb_PF->connect({ BUS_a_load_PF });
	//shunt_SR_bcb_PF->connect({ BUS_b_load_PF });
	...
	trafo->setParameters( generic_model_B_A.nomPhPhVoltRMS_G1, generic_model_B_A.Vnom, generic_model_B_A.nomPower_G1,
                     (generic_model_B_A.nomPhPhVoltRMS_G1/generic_model_B_A.Vnom)*(1 + (2.5/100)*0), 5.0*30*0, Math::singlePhaseParameterToThreePhase(3.64157728),
	...
	auto dummy_load_bus_psha_EMT = EMT::Ph3::RXLoad::make("dummy_load_bus_psha", Logger::Level::debug);
	dummy_load_bus_psha_EMT->setParameters(CPS::Math::singlePhasePowerToThreePhase(1e-12), CPS::Math::singlePhasePowerToThreePhase(1e-12), generic_model_B_A.Vnom);
	...
	dummy_load_bus_psha_EMT->connect({ BUS_psha_EMT });
	// faultEMT->connect({EMT::SimNode::GND , n3EMT }); //Load bus

	auto systemEMT = SystemTopology(50,
			SystemNodeList{BUS_gas_EMT, BUS_a_EMT, BUS_b_EMT, BUS_a_load_EMT, BUS_b_load_EMT, BUS_psha_EMT},
			SystemComponentList{GEN_gas_EMT, cable_EMT, line3_EMT, trafo, breaker_a_EMT, breaker_b_EMT, load_bus_a_EMT, load_bus_b_EMT, dummy_load_bus_psha_EMT});
